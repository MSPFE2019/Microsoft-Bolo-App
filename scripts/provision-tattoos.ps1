# Adds the optional "Scars, marks & tattoos" column to the person BOLO table.
#
# The app ships this field, which needs a real Dataverse column to store into.
# Run this once against an environment that was created before the field
# existed. It is safe to re-run: an existing column is left alone.
#
# Usage: pwsh scripts/provision-tattoos.ps1 -Token <bearer token>
param(
...
# A Memo column, because officers list several marks one per line and a single
# text column would neither hold the length nor keep the line breaks.
$body = @{
  "@odata.type"     = "Microsoft.Dynamics.CRM.MemoAttributeMetadata"
  SchemaName        = "new_Tattoos"
  DisplayName       = @{ LocalizedLabels = @(@{ Label = "Scars, marks & tattoos"; LanguageCode = 1033 }) }
...
Write-Host "Adding new_Tattoos to new_personbolo..." -ForegroundColor Cyan
try {
