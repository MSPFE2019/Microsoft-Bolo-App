...
export type FieldType = "text" | "textarea" | "select" | "multiselect" | "photo";
...
export interface FieldDef {
  ...
  placeholder?: string;
  /**
   * Built-in fields map to a dedicated BoloRecord property and a real column
   * that already exists. They can be hidden and reordered but never deleted,
   * because removing one would orphan data the rest of the app reads.
   */
  builtin: boolean;
  /**
   * Custom fields live in BoloRecord.custom and need a real Dataverse column
   * provisioned by scripts/provision-custom-fields.ps1 before they can be
   * saved. Until then they are shown as pending and skipped on write.
   */
  logicalName?: string;
  provisioned?: boolean;
}
...
/** Fields whose values the app itself reads, so they may never be deleted. */
...
/** A custom field can only round-trip once its Dataverse column exists. */
export function isPending(field: FieldDef): boolean {
  return !field.builtin && !field.provisioned;
}

export function slugToLogicalName(label: string): string {
  const slug = label
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "")
    .slice(0, 40);
  return `new_cf${slug || "field"}`;
}

/**
 * Reconciles a saved config against the current build's catalogue so that
 * shipping a new built-in field doesn't require an admin edit, and a renamed
 * or removed one doesn't leave a dangling entry.
 */
export function mergeWithBuiltins(saved: FieldConfig | null): FieldConfig {
  if (!saved?.fields?.length) return DEFAULT_CONFIG;
  ...
  for (const field of saved.fields) {
    if (field.builtin) {
      ...
      if (!builtin) continue;
      // Admins own presentation; the build owns type/scope so the renderer and
      // the record shape can never disagree.
      merged.push({
        ...builtin,
  ...
  for (const builtin of BUILTIN_FIELDS) {
    if (!savedByKey.has(builtin.key)) merged.push(builtin);
  }
